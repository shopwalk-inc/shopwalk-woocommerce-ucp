...
class Testable_Shopwalk_WC_Sync extends Shopwalk_WC_Sync {
	...
	public function test_double_delete_sends_only_one_event(): void {
		self::$options['shopwalk_wc_plugin_key']  = 'sk_test_abc123';
		self::$options['shopwalk_wc_enable_sync'] = 'yes';
		...
		$sync->flush_responses = array( true, true );
		...
		$ref->setValue( null, array( 42 ) ); // Mark product 42 as already deleted

		// Second call with same ID should be skipped — push_to_queue not called
		$queue_before = self::$options['shopwalk_wc_sync_queue'] ?? array();
		$sync->expose_push_to_queue( json_encode( array( 'event_type' => 'should_not_appear' ) ) );
	...
	public function test_flush_works_when_invoked_by_cron(): void {
		self::$options['shopwalk_wc_plugin_key'] = 'sk_test_abc123';
		self::$options['shopwalk_wc_sync_queue'] = array(
			json_encode(
				array(
					'event_type' => 'product.upsert',
					'product'    => array( 'external_id' => '77' ),
				)
			),
		);

		$sync                  = $this->makeSyncInstance();
		$sync->flush_responses = array( true );
...
class SyncTest extends TestCase {
	...
	public function test_api_down_queues_event_silently(): void {
		...
		$queue = Testable_Shopwalk_WC_Sync::$options['shopwalk_wc_sync_queue'] ?? array();
	...
	public function test_queue_cap_drops_silently_when_full(): void {
		Testable_Shopwalk_WC_Sync::$options['shopwalk_wc_sync_queue'] =
			array_fill( 0, 500, json_encode( array( 'event_type' => 'noop' ) ) );

		$sync = $this->makeSyncInstance();

		try {
			$sync->expose_push_to_queue( json_encode( array( 'event_type' => 'product.upsert' ) ) );
		} catch ( \Throwable $e ) {
	...
	public function test_flush_retries_queued_events_on_recovery(): void {
		...
		Testable_Shopwalk_WC_Sync::$options['shopwalk_wc_sync_queue'] = array(
			json_encode(
				array(
					'event_type' => 'product.upsert',
					'product'    => array( 'external_id' => '1' ),
				)
			),
			json_encode(
				array(
					'event_type' => 'product.upsert',
					'product'    => array( 'external_id' => '2' ),
				)
			),
			json_encode(
				array(
					'event_type' => 'product.delete',
					'product'    => array( 'external_id' => '3' ),
				)
			),
		);

		$sync                  = $this->makeSyncInstance();
		$sync->flush_responses = array( true, true, true );

		$sync->expose_flush();

		$remaining = Testable_Shopwalk_WC_Sync::$options['shopwalk_wc_sync_queue'] ?? array();
	...
	public function test_flush_keeps_failed_events_at_front_of_queue(): void {
		...
		$sync                  = $this->makeSyncInstance();
		$sync->flush_responses = array( true, false, true ); // #2 fails
	...
	public function test_401_does_not_queue_and_marks_key_invalid(): void {
		...
		$sync                     = $this->makeSyncInstance();
		$sync->flush_status_codes = array( 401 ); // First event gets a 401
		$sync->flush_responses    = array( false ); // irrelevant but set
		...
		$this->assertEquals(
			1,
			Testable_Shopwalk_WC_Sync::$options['shopwalk_wc_key_invalid'] ?? 0,
	...
	public function test_flush_handles_mixed_event_types(): void {
		...
		$sync                  = $this->makeSyncInstance();
		$sync->flush_responses = array( true, true, true, true, true );

		$sync->expose_flush();

		$this->assertEmpty( Testable_Shopwalk_WC_Sync::$options['shopwalk_wc_sync_queue'] ?? array() );
	...
	public function test_flush_respects_batch_limit(): void {
		...
		Testable_Shopwalk_WC_Sync::$options['shopwalk_wc_sync_queue'] = array_map(
			fn( $i ) => json_encode(
				array(
					'event_type' => 'product.upsert',
					'product'    => array( 'external_id' => (string) $i ),
				)
			),
			range( 1, 75 )
		);
	...
	public function test_double_delete_sends_only_one_event(): void {
		...
		// Second call with same ID should be skipped — push_to_queue not called
		$queue_before = Testable_Shopwalk_WC_Sync::$options['shopwalk_wc_sync_queue'] ?? array();
		$sync->expose_push_to_queue( json_encode( array( 'event_type' => 'should_not_appear' ) ) );
