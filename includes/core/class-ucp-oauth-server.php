...
final class UCP_OAuth_Server {
	...
	private static function scope_labels(): array {
		return array(
			'openid'        => __( 'Verify your account identity', 'ucp-for-woocommerce' ),
			'profile'       => __( 'Read your basic profile (name, email)', 'ucp-for-woocommerce' ),
			'email'         => __( 'Read your email address', 'ucp-for-woocommerce' ),
			'ucp:checkout'  => __( 'Place orders on your behalf at this store', 'ucp-for-woocommerce' ),
			'ucp:orders'    => __( 'Read your order history at this store', 'ucp-for-woocommerce' ),
			'ucp:webhooks'  => __( 'Receive notifications about your orders', 'ucp-for-woocommerce' ),
		);
	}
	...
	private static function render_consent_page( array $ctx ): string {
		...
		$title         = esc_html__( 'Authorize access', 'ucp-for-woocommerce' );
		$intro         = esc_html(
			sprintf(
				/* translators: %s: agent / client display name */
				__( '%s is requesting access to your account at this store.', 'ucp-for-woocommerce' ),
				$client_name
			)
		);
		$redirect_lbl  = esc_html__( 'After you decide, you will be returned to:', 'ucp-for-woocommerce' );
		$scopes_lbl    = esc_html__( 'It will be allowed to:', 'ucp-for-woocommerce' );
		$approve_label = esc_html__( 'Approve', 'ucp-for-woocommerce' );
		$deny_label    = esc_html__( 'Deny', 'ucp-for-woocommerce' );
