# Release checklist — WooCommerce UCP
...
1. **Bump version in the same commit as the change** (per the long-standing
   ...
   - `woocommerce-ucp.php` header `Version:`
...
- [ ] `languages/woocommerce-ucp.pot` is up to date:
      ```bash
      wp i18n make-pot . languages/woocommerce-ucp.pot \
          --domain=woocommerce-ucp \
...
wp plugin install ./woocommerce-ucp.zip          # or upload via Plugins → Add New
# UI: Tools → Plugin Check → pick "WooCommerce UCP" → Check it!
# CLI:
wp plugin check woocommerce-ucp
```
...
5. On approval, you'll get **SVN credentials** at
   `https://plugins.svn.wordpress.org/woocommerce-ucp/`.
...
svn co https://plugins.svn.wordpress.org/woocommerce-ucp wp-svn
cd wp-svn

# 2. Pull the GitHub release zip for v<X.Y.Z> and unpack into trunk/
rm -rf trunk
unzip ~/Downloads/woocommerce-ucp-<X.Y.Z>.zip -d /tmp/wcucp
mv /tmp/wcucp/woocommerce-ucp trunk
svn add --force trunk
